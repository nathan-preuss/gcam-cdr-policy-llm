    ...
    useEffect(() => {
        getBedrockAgents().then((agents) => {
            ...
            const agOptions = ags.map(ag => {
                console.log(ag)
                return ({
                    label: `${ag.agentName} (ID: ${ag.agentId}) ${ag.agentStatus} Last Updated: ${formatDates(ag.updatedAt)}`,
                    value: ag,
                    iconName: "gen-ai",
                    description: ag.status,
                    tags: [ `Alias: ${ag.alias.agentAliasId} / ${ag.alias.agentAliasStatus}`, "Last Updated: " + formatDates(ag.alias.updatedAt)]
                })
            })
            setAgents(agOptions)
